...
import { 
  ...
  WorkoutSession, ExerciseEntry, UserProfile, GoalSettings
} from '../../lib/types';
import { generateHeuristicProgram } from '../../lib/training/generator';
import { getExerciseMetadata } from '../../lib/training/catalog';
import { calculateNextTarget, ProgressionTarget } from '../../lib/training/progression';
import { Dumbbell, Play, Sparkles, ChevronRight, ArrowLeft, Save } from 'lucide-react';
...
export default function LiftingPage() {
  ...
  useEffect(() => {
    const init = async () => {
      const [p, progs, state] = await Promise.all([
    ...
    return (
      <div className="space-y-6">
        <header className="flex items-center space-x-4">
          <button onClick={() => setView('list')} className="p-2 bg-white border rounded-full shadow-sm"><ArrowLeft size={20} /></button>
              ...
              <div className="mt-4 space-y-3">
                {currentWorkout.logs[ex.id].map((set: any, idx: number) => (
                  <div key={idx} className="flex gap-2">
                     <input type="number" value={set.weight} onChange={(e) => { const n = {...currentWorkout.logs}; n[ex.id][idx].weight = parseFloat(e.target.value) || 0; setCurrentWorkout({...currentWorkout, logs: n}) }} className="w-20 p-2 border rounded-xl bg-gray-50" placeholder="lb" />
                     <input type="number" value={set.reps} onChange={(e) => { const n = {...currentWorkout.logs}; n[ex.id][idx].reps = parseInt(e.target.value) || 0; setCurrentWorkout({...currentWorkout, logs: n}) }} className="w-20 p-2 border rounded-xl bg-gray-50" placeholder="reps" />
                  </div>
                ))}
              </div>
            </div>
        ...
        <button onClick={() => setView('list')} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-lg">Save Session</button>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      {activeState && activeProgram ? (
        <section className="p-6 rounded-3xl bg-blue-600 text-white shadow-xl">
          ...
          <button onClick={handleStartWorkout} className="mt-6 w-full bg-white text-blue-700 font-bold py-4 rounded-2xl flex items-center justify-center space-x-2 shadow-sm">
        ...
        <div className="bg-white p-12 rounded-3xl border-2 border-dashed border-gray-200 text-center flex flex-col items-center">
          <Dumbbell className="text-gray-200 mb-4" size={48} /><p className="text-gray-500 font-medium">Select a program to begin training.</p>
        </div>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {programs.map(p => (
          <div key={p.id} className="bg-white p-5 rounded-2xl border shadow-sm flex items-center justify-between cursor-pointer hover:border-blue-400 transition-all">
            <div><h4 className="font-bold">{p.name}</h4><p className="text-xs text-gray-400">{p.lengthWeeks} weeks</p></div><ChevronRight size={18} className="text-gray-300" />
          </div>
        ))}
      </div>
    </div>
